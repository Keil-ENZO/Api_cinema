...
const url =
  ...
  function trailer(id) {
    ...
    // Vérifier si le trailer a déjà été chargé pour cette carte
    if (cardHoverState[id]) {
      return; // Sortir de la fonction si le trailer a déjà été chargé
    }
    ...
    fetch(`https://api.themoviedb.org/3/movie/${id}/videos?api_key=${apiKey}`)
      ...
      .then((data) => {
        ...
        cardHoverState[id] = true; // Définir l'indicateur pour indiquer que le trailer a été chargé
      });
  }
